<template>
  <div>
    <b-button v-b-modal.modal-prevent-closing>신규 신청</b-button>
    <b-modal
...
export default {
  ...
  data() {
    return {
      ...
      to: this.getFormatDate(new Date(new Date().getTime() + (24*60*60*1000))), 
      when: this.getFormatDate(new Date()), 
    }
  },
  computed: {
    dayDiff() {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      ApplyService.doApply(json).then(res => {
        if(res == null)
          return;
        // Hide the modal manually
        this.$nextTick(() => {
          this.$bvModal.hide('modal-prevent-closing')
          this.$emit('childData', '등록')
        })
      })      
    },    
    /**
